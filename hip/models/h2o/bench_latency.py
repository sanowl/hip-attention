...
def main_latency_benchmark():
    try:
        ...
        logger.info(f"Initial CUDA memory: {initial_memory/1e6:.2f}MB")
        ...
        # Better head size handling
        q, k, v, _, cos, sin = load_checkouts(idx=0, window=40, seq_len=CHUNK_LEN)
        head_size = q.shape[0]  # Moved from below and properly integrated
        
        # Add validation
        assert head_size > 0, "Head size must be positive"
        
        HID = q.shape[-1]
        original_head_size = head_size  # Store original for later reference
        
        # Update head size handling
        if args.head_size > 0:
            if args.head_size != original_head_size:
                head_reps = int(math.ceil(args.head_size / original_head_size))
        ...
        sin = sin.cuda()
        if METHOD in ['hip1.1']:
            ##TODO is "q_quant" is not accessedPylance is not acccsed
        ...
        logger.info(f"Final CUDA memory: {final_memory/1e6:.2f}MB")
